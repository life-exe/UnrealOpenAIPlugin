...
#include "ResumeFineTuningJobAction.generated.h"

DECLARE_DYNAMIC_MULTICAST_DELEGATE_ThreeParams(FOnResumeFineTuningJob, const FFineTuningJobObjectResponse&, Response,
    const FOpenAIResponseMetadata&, ResponseMetadata, const FOpenAIError&, RawError);

UCLASS()
class OPENAI_API UResumeFineTuningJobAction : public UOpenAIActionBase
{
    GENERATED_BODY()

public:
    UPROPERTY(BlueprintAssignable)
    FOnResumeFineTuningJob OnCompleted;
...
private:
    ...
    static UResumeFineTuningJobAction* ResumeFineTuningJob(
        const FString& FineTuningJobID, const FOpenAIAuth& Auth, const FString& URLOverride);

    void OnResumeFineTuningJobCompleted(const FFineTuningJobObjectResponse& Response, const FOpenAIResponseMetadata& ResponseMetadata);
